#!/usr/bin/env bash
# One command for Signal Intelligence: API + console together.
#
#   ./ops/dev.sh          → http://localhost:3000
#
...
if nc -z localhost "${TEMPORAL_PORT:-7233}" 2>/dev/null; then
  echo "→ worker  connected to Temporal :${TEMPORAL_PORT:-7233}"
  .venv/bin/python -m app.workflows.worker &
else
  echo "→ worker  skipped (no Temporal on :${TEMPORAL_PORT:-7233}) — the API runs the loop in-process"
fi
